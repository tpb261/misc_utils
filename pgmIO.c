...
#define UNIT_TEST 1
#if UNIT_TEST
#define pgm_dbg_printf printf
#else
#define pgm_dbg_printf(...) /* do nothing */
#endif
...
{
    unsigned short chans;  /**< number of channels */
...
int readData (
    ...
    int pgmType)
{
    if(pgmType == 5 || pgmType == 6)
    {
        return (fread (pImg->cData, pImg->chans*pImg->cols*pImg->bpp, pImg->rows, fp) == pImg->rows);
    }
    else if (pgmType == 2)
    {
        int i;
        for(i = 0; i<pImg->rows*pImg->cols && !feof (fp); i++)
        {
            fscanf (fp, "%d", &pImg->cData[i]);
        }
        if( i != pImg->rows*pImg->cols)
        {
            return 0;
        }
    }
    ...
    )
{
    ...
    if(pgmType == 2 || pgmType == 5) chans = 1;
    else if(pgmType == 3 || pgmType == 6) chans = 1;

    while(!feof (fp))
    {
        int readRows = rows;
        pImg         = (image_t*)malloc (sizeof(image_t));
        pImg->cData  = (char*)malloc (cols*rows*bpp*chans);
        pImg->rows   = rows;
        pImg->cols   = cols;
        pImg->bpp    = bpp;
        pImg->chans  = chans;
        if(0 == readData (fp, pImg, pgmType))
            goto cleanup;
        else
        {
            *pImgs = (image_t**)realloc (*pImgs, sizeof(image_t*)*(*numFrames+1));
            (*numFrames)++;
